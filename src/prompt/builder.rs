...
1. 修炼进度是否变化？叙事中是否有修炼、吐纳、感悟、突破等描写？
   → realm_progress (0.0~1.0 的增量，小幅进步 0.02~0.05，显著进步 0.08~0.15，突破填 1.0)
   如果没有任何修炼描写，填 null

2. 境界名称是否变化？叙事中是否明确出现了新的境界名称（如"练气期中期""筑基期初期"）？
...
3. 灵力是否变化？叙事中是否有消耗灵力（施法、战斗、御器）、恢复灵力（休息、服药、吸收灵气）的描写？
   → qi_delta (负数表示消耗，正数表示恢复)
   如果没有灵力变化的描写，填 null

4. 灵力是否被设定为某个精确值（如突破后灵力回满）？
   → qi_set (具体数值)
   如果不是精确设定，填 null

5. 灵力上限是否变化（如突破境界后上限提升）？
   → max_qi_delta
   如果没有，填 null

6. 灵石数量是否变化？叙事中是否有获得灵石、花费灵石的描写？
   → spirit_stones_delta (获得为正，花费为负)
   如果没有，填 null

7. 剑道是否提升？叙事中是否有剑术、剑气、剑法相关的修炼或领悟描写？
   → sword_art_delta (+1~+3)
   如果没有，填 null

8. 术法是否提升？叙事中是否有术法、法术、神通、符箓相关的修炼或领悟描写？
   → spell_art_delta (+1~+3)
   如果没有，填 null

9. 气血是否提升？叙事中是否有肉身、体魄、炼体相关的修炼或强化描写？
   → blood_qi_delta (+1~+3)
   如果没有，填 null

10. 神魂是否提升？叙事中是否有魂魄、元神、精神相关的修炼或强化描写？
    → spirit_soul_delta (+1~+3)
    如果没有，填 null

11. 神识是否提升？叙事中是否有神识、灵觉、感知相关的修炼或突破描写？
    → divine_sense_delta (+1~+3)
    如果没有，填 null

12. 道心是否提升？叙事中是否有道心、心性、意志相关的感悟或突破描写？
    → dao_heart_delta (+1~+3)
    如果没有，填 null

13. 是否习得了新功法？叙事中是否出现了新的功法名称（如师父传授、自行领悟、获得秘籍）？
    → add_techniques: [{{"name":"功法名","tier":"黄阶/玄阶/地阶/天阶","tech_type":"攻击/防御/身法/心法","proficiency":0.1}}]
    注意：如果功法名已存在于"当前已有功法"列表中，不要重复添加，填 []
    如果没有新功法，填 []

14. 是否获得了新物品？叙事中是否出现了新的物品（如捡到、收到、购买、发现）？
...
16. 是否消耗/使用了物品？叙事中是否描写了使用物品（如服药、使用符箓、消耗材料）？
    → consume_items: [{{"name":"物品名（必须精确匹配当前已有物品名）","quantity":消耗数量（未明确则默认1）}}]
    如果没有消耗物品，填 []

17. 当前位置是否变更？叙事中玩家是否移动到了新的地点？
    → set_current_location: "新地点全名"
    如果没有，填 null

18. 是否发现了新地点？玩家是否初次到达一个不在"已探索地点"列表中的地点？
    → new_locations: ["新地点名"]
    注意：如果该地点已在已探索列表中，填 []
    如果没有新地点，填 []

19. 人物好感是否变化？叙事中是否有人物对玩家态度变化的描写（如更加信任、冷淡、尊敬、厌恶等）？
    → relationship_changes: [{{"name":"人物名（必须精确匹配当前已有关系名）","affinity_delta":±数值}}]
    小幅变化填 ±3~±10，明显变化填 ±10~±20
    如果没有好感变化，填 []

20. 已知人物是否获得了新名字？叙事中是否揭示了某个人物的真名（如之前叫"未知壮汉"，现在知道叫"赵铁柱"）？
    → rename_relationships: [{{"old_name":"旧名字（当前关系列表中的名字）","new_name":"新名字"}}]
    注意：不要添加新人物！用这个字段来改名
    如果没有改名，填 []

21. 任务是否有变化？叙事中是否有任务完成、接到新任务的描写？
